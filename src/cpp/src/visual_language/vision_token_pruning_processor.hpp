...
struct PruningContext {
    ...
    int64_t slice_start_token_id = -1;   // For MiniCPM slices, -1 means not used
    int64_t slice_end_token_id = -1;     // For MiniCPM slices, -1 means not used
...
public:
    ...
    /**
     * @brief Generate keep flags for 1D position encoding models (MiniCPM, LLaVA).
     * Only generates keep_flags without modifying position_ids (position_ids will be auto-generated by VLMPipeline).
     */
    void generate_keep_flags_1d(const ov::Tensor& input_ids,
                                int64_t vision_start_token_id,
                                int64_t image_pad_token_id,
                                const std::vector<std::array<size_t, 3>>& reordered_images_grid_thw,
                                const std::vector<std::vector<size_t>>& kept_indices_per_image,
    ...
    /**
     * @brief Update 1D position IDs for models that need explicit position_ids modification (legacy).
    ...
    ov::Tensor generate_pruned_input_ids(const ov::Tensor& input_ids,
                                         ...
                                         int64_t slice_start_token_id = -1,
                                         int64_t slice_end_token_id = -1) const;
