...
/**
 * @brief Split video embeddings into individual frame embeddings
 * @param video_embeddings Video embeddings [T*H*W, hidden_dim] for a single video
 * @param grid_t Number of frames (temporal dimension)
 * @param grid_h Height dimension
 * @param grid_w Width dimension
    ...
    if (is_3d_encoding) {
        ...
        if (seq_len <= max_display_lines) {
            for (size_t i = 0; i < seq_len; ++i) {
                ...
                table_stream << std::setw(7) << i << " | " << std::setw(8) << token_id << " | " << std::setw(8)
                             << temporal_val << " | " << std::setw(6) << height_val << " | " << std::setw(5)
                             << width_val << "\n";
            }
        } else {
                ...
                for (size_t orig_idx = 0; orig_idx < original_seq_len && pruned_idx < pruned_seq_len; ++orig_idx) {
                    ...
                    if (inside_vision && token_id == context.vision_pad_token_id) {
                        ...
                        if (region_idx < result.keep_flags_per_region.size() &&
                            token_in_region < result.keep_flags_per_region[region_idx].size() &&
                            result.keep_flags_per_region[region_idx][token_in_region]) {
                            pruned_to_original[pruned_idx++] = orig_idx;
                        }
                        token_in_region++;
                        continue;
            ...
            for (size_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
                ...
                for (size_t pruned_idx = 0; pruned_idx < pruned_seq_len; ++pruned_idx) {
                    ...
                    if (inside_vision && token_id == context.vision_pad_token_id) {
                        ...
                        size_t frame_idx = 0;
                        for (size_t i = 0; i < frame_metadata.size(); ++i) {
                            if (vision_token_count < cumulative_tokens + tokens_per_frame[i]) {
                                frame_idx = i;
                                break;
                            }
