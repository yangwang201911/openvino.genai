...
EncodedImage VisionEncoderLLaVA::encode_with_pruning(
    ...
    try {
        ...
        if (current_config.has_value() && current_config->num_visual_tokens != num_visual_tokens) {
            // Create new CDPruner with updated token count
            current_config->num_visual_tokens = num_visual_tokens;

            // Validate the new configuration
            if (current_config->num_visual_tokens == 0) {
