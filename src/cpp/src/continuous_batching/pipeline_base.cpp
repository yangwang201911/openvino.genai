...
ContinuousBatchingPipeline::IContinuousBatchingPipeline::generate(
    ...
    if (m_is_chat_conversation) {
        ...
        encoded_images = m_inputs_embedder->encode_images(rgbs, vision_config, sampling_params[0].is_video);
        ...
        for (size_t i = 0; i < prompts.size(); i++) {
            ...
            const auto encoded_images = m_inputs_embedder->encode_images(rgbs, vision_config, sampling_params[i].is_video);
...
ContinuousBatchingPipeline::IContinuousBatchingPipeline::add_request(uint64_t request_id,
    ...
    ov::Tensor inputs;
    {
        ...
        const auto encoded_images = m_inputs_embedder->encode_images(rgbs, vision_config, sampling_params.is_video);
